...
class ArticlePagingSource : PagingSource<Int, Article>() {
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, Article> {
        ...
        if (start != STARTING_KEY) delay(LOAD_DELAY_MILLIS) //a little delay to simulate the load
    ...
    override fun getRefreshKey(state: PagingState<Int, Article>): Int? {
        ...
        val anchorPosition = state.anchorPosition ?: return null//load most recently accessed index in the list
        val article = state.closestItemToPosition(anchorPosition) ?: return null//load closest item to the last accessed index in the list
