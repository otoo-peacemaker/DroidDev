...
class AutoCompleteActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val theme = intent.getIntExtra(MapsActivity.THEME_RES_ID_EXTRA, 0)
        if (theme != 0) {
            setTheme(theme)
        }
        ...
        useTypeFilterCheckBox.setOnCheckedChangeListener { _, isChecked: Boolean ->
            typeFilterSpinner.isEnabled = isChecked
        }
    ...
    private fun findAutocompletePredictions() {
        ...
        task.addOnCompleteListener { response: Task<FindAutocompletePredictionsResponse>? ->
            setLoading(
                false
            )
        }
    ...
    private val mode: AutocompleteActivityMode
        get() {
            val isOverlayMode =
                findViewById<CheckBox>(R.id.autocomplete_activity_overlay_mode).isChecked
